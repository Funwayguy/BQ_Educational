...
public class UpdateNotification
{
	...
	{
		if(!BQ_Educational.proxy.isClient() || hasChecked)
		{
			return;
		}
		...
		if(BQ_Educational.VERSION == "BQ_EDU_" + "VER")
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "THIS COPY OF " + BQ_Educational.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		...
		try
		{
			...
			int verStat = compareVersions(BQ_Educational.VERSION, version);
			
			if(verStat == -1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Update " + version + " of " + BQ_Educational.NAME + " available!"));
			...
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.YELLOW + BQ_Educational.NAME + " " + BQ_Educational.VERSION + " is up to date"));
			} else if(verStat == 1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + BQ_Educational.NAME + " " + BQ_Educational.VERSION + " is a debug build"));
			} else if(verStat == -2)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "An error has occured while checking " + BQ_Educational.NAME + " version!"));
			}
